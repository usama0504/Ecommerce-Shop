...
<template>
  <div class="mb-6 sm:mb-8">
    <h2 class="text-base sm:text-lg font-bold mb-3 sm:mb-4 flex justify-between items-center text-zinc-900 dark:text-zinc-100">
    ...
    <div class="flex gap-3 sm:gap-4 overflow-x-auto pb-2 sm:pb-3 scrollbar-none [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
      <button 
      ...
      >
        ...
        <span class="text-[11px] sm:text-xs font-semibold whitespace-nowrap text-zinc-600 dark:text-zinc-300 capitalize tracking-tight">{{ cat.name }}</span>
      </button>
    </div>
  </div>
</template>